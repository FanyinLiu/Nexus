...
export function LegacyPanelView({
  settings,
  petModel,
  memory,
  pet,
  ...
  const [panelSessionStartedAt] = useState(() => new Date())
  ...
  const visibleMessages = useMemo(
    () => image4PreviewMode && image4ChatPreviewMode
      ? buildImage4ChatPreviewMessages(panelClock, image4ChatPreviewVariant)
      : chat.messages,
    [chat.messages, image4ChatPreviewMode, image4ChatPreviewVariant, image4PreviewMode, panelClock],
  )
  ...
  const image4ElapsedBucket = image4PreviewMode
    ? 'about_half_hour'
    : getPanelElapsedBucket(panelSessionStartedAt.getTime(), panelClock.getTime())
  ...
  const conversationMessages = visibleMessages.filter((message) => message.role !== 'system')
  const latestUserIndex = conversationMessages.map((message) => message.role).lastIndexOf('user')
  const latestAssistantIndex = conversationMessages.map((message) => message.role).lastIndexOf('assistant')
  const latestUserMessage = latestUserIndex >= 0 ? conversationMessages[latestUserIndex] : undefined
  // Never place an older assistant reply underneath a newer unanswered user turn.
  const latestAssistantMessage = latestAssistantIndex > latestUserIndex
    ? conversationMessages[latestAssistantIndex]
    : undefined
  const recapAssistantMessage = image4CompanionState.mode === 'speaking' ? undefined : latestAssistantMessage
  const conversationRecap = [latestUserMessage, recapAssistantMessage].filter(
    (message): message is NonNullable<typeof message> => Boolean(message),
  )
  const conversationSubtitle = voice.voiceState === 'listening'
    ? voice.liveTranscript.trim()
    : voice.voiceState === 'speaking' || image4CompanionState.mode === 'speaking'
  ...
  const notificationReplyHintLabel = activeNotificationReply
    ...
    : null
  const composerPlaceholder = ti('panel.composer.placeholder_compact')
        ...
        {panelCollapsed ? (
          ...
          <div className="panel-window__shell panel-window__shell--image4 image4-layout">
            ...
            >
              ...
              <div className="image4-live2d-stage" aria-label={`${settings.companionName} Live2D`}>
                <Suspense fallback={null}>
                  <Live2DCanvas
                    ...
                    placement="pet-stage"
                    paused={Boolean(onboardingGuide)}
                  />
                </Suspense>
                {conversationSubtitle ? (
                  <p className="image4-live2d-subtitle">{shorten(conversationSubtitle, 22)}</p>
                ) : null}
              </div>

              {conversationRecap.length ? (
                <div className="image4-conversation-recap" aria-label={ti('panel.messages.aria_label')}>
                  {conversationRecap.map((message) => (
                    <div className={`image4-conversation-recap__line is-${message.role}`} key={message.id}>
                      <strong>{message.role === 'user' ? '我' : settings.companionName}</strong>
                      <span>{shorten(message.content, 58)}</span>
                    </div>
                  ))}
                </div>
              ) : null}
              ...
              <p className="sr-only" role="status" aria-live="polite" aria-atomic="true">
                {latestAssistantMessage ? `${settings.companionName}：${latestAssistantMessage.content}` : ''}
              </p>

              <div
                ref={messageListRef}
                className={`message-list companion-chat__messages image4-message-list image4-message-list--archive ${visibleMessages.length ? '' : 'is-empty'}`}
                aria-live="polite"
...
export default LegacyPanelView
