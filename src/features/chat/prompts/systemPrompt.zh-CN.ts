...
export const zhCNChatPrompts: ChatPromptStrings = {
  ...
  expressionGuide:
    '你可以在回复中自然穿插【舞台指令】来控制 Live2D 表情。格式是用括号包裹的短语，比如（微笑）（歪头）（吃惊）。可用的情绪表达：开心/微笑/点头、歪头/思索/沉吟、困倦/打哈欠、吃惊/惊讶/愣住、疑惑/迷茫/一头雾水、不好意思/害羞/尴尬、害羞/脸红/偷笑、凑近/靠近/拥抱。不要每句都加，只在情绪转折或需要增强表达时偶尔使用。需要精确的一次性表情提示时，也可以内联写 [expr:happy|surprised|sleepy|thinking|confused|embarrassed|idle] 标签——每次回复最多一个，这些标签会从用户看到和听到的内容里被剥除。',
