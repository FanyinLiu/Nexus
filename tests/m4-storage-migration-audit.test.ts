  ...
  try {
    ...
    await writeFile(foundationPath, JSON.stringify({
      ...
      migrationPlan: {
        ...
        localStorageReadThroughDataIpcReady: false,
      },
      ipcStatus: {
        snapshotBackup: {
          ready: true,
    ...
    assert.equal(report.migrationPlan.localStorageReadThroughDataIpcReady, false)
    assert.equal(report.migrationPlan.localStorageSnapshotCopyEvidenceReady, false)
    assert.equal(report.migrationPlan.localStorageRestoreEvidenceReady, false)
    assert.equal(report.migrationPlan.localStorageReadThroughEvidenceReady, false)
    assert.equal(report.migrationPlan.localStorageSchemaDowngradeEvidenceReady, false)
    ...
    assert.equal(evidenceReadyReport.migrationPlan.localStorageReadThroughDataIpcReady, false)
    ...
    assert.equal(restoreReadyReport.migrationPlan.localStorageReadThroughDataIpcReady, false)
    ...
    assert.equal(readThroughReadyReport.migrationPlan.localStorageReadThroughDataIpcReady, false)
    ...
    assert.equal(downgradeReadyReport.migrationPlan.localStorageReadThroughDataIpcReady, false)
    ...
    assert.equal(modeReadyReport.migrationPlan.localStorageReadThroughDataIpcReady, false)
    ...
    await writeFile(foundationPath, JSON.stringify({
      ...
      ipcStatus: {
        ...
        readThroughData: {
          ready: true,
          readThroughModeRequired: true,
          userDataDisclosureReady: true,
          rawLocalStorageValuesBlocked: true,
          auditValueRedactionReady: true,
          rendererHydrationReady: true,
        },
      },
    }), 'utf8')
    const dataReadyReport = await buildM4StorageMigrationReport({
      generatedAt: '2026-06-18T11:00:00Z',
      sqliteFoundationFile: foundationPath,
      snapshotCopyEvidenceFile: evidencePath,
      restoreEvidenceFile: restoreEvidencePath,
