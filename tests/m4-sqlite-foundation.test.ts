...
test('m4 sqlite foundation report is private-safe and non-migrating', async () => {
  ...
  try {
    ...
    assert.equal(report.migrationPlan.localStorageReadThroughDataIpcReady, true)
    ...
    assert.equal(report.ipcStatus.readThroughData.ready, true)
    assert.equal(report.ipcStatus.readThroughData.channel, 'storage:read-through-data')
    assert.equal(report.ipcStatus.readThroughData.preloadExposed, true)
    assert.equal(report.ipcStatus.readThroughData.handlerRegistered, true)
    assert.equal(report.ipcStatus.readThroughData.trustedSenderCheck, true)
    assert.equal(report.ipcStatus.readThroughData.requestValidationReady, true)
    assert.equal(report.ipcStatus.readThroughData.responseValidationReady, true)
    assert.equal(report.ipcStatus.readThroughData.rendererTypeDeclared, true)
    assert.equal(report.ipcStatus.readThroughData.readThroughModeRequired, true)
    assert.equal(report.ipcStatus.readThroughData.userDataDisclosureReady, true)
    assert.equal(report.ipcStatus.readThroughData.rawLocalStorageValuesBlocked, true)
    assert.equal(report.ipcStatus.readThroughData.sourcePreservationReady, true)
    assert.equal(report.ipcStatus.readThroughData.runtimeMigrationDisabled, true)
    assert.equal(report.ipcStatus.readThroughData.auditValueRedactionReady, true)
    assert.equal(report.ipcStatus.readThroughData.rendererHydrationReady, true)
