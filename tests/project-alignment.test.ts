...
test('v0.4 desktop companion awareness hardening stays documented', () => {
  ...
  assert.match(v04Plan, /current\s+active v0\.4\.x slice is `v0\.4\.1` Coarse Time Language/)
  assert.match(v04Plan, /Do not expand `v0\.4\.1` into proactive check-ins/)
  assert.match(v04Plan, /Future candidate after v0\.4\.0 stable: v0\.4\.2/)
