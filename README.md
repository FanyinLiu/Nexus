...
> **主题：记忆看得见，伙伴也动起来。** 中文说明见 [RELEASE-NOTES-v0.3.5.zh-CN.md](docs/RELEASE-NOTES-v0.3.5.zh-CN.md)，英文完整说明见 [RELEASE-NOTES-v0.3.5.md](docs/RELEASE-NOTES-v0.3.5.md)。

以前 Nexus 可以“记住”，但用户不容易知道它到底想起了什么；桌面头像也还不够能表达“她现在在做什么”。0.3.5 把这两个体验一起推到台面上：回复会提示是否用了记忆，可以展开看来源，点进 Memory 页还能高亮对应条目；用户也可以暂停全部记忆或单条记忆。同时，Companion Profile 可以直接预览桌面伙伴的待机、思考、聆听、说话、等待、错误和离线状态。这版不把 Nexus 做成替你干活的 Codex 式智能体，而是让陪伴记忆变得本地、可看、可停、可回滚，也让桌面存在感更容易被看懂。

一句话记住 0.3.5：

- **Nexus 现在能说明这次回复用了哪些记忆。**
- **你可以回到 Memory 页看到、暂停、编辑或删除这些记忆。**
- **桌面伙伴现在能展示待机、思考、聆听、说话、等待、错误和离线状态。**
