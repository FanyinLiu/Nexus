...
> **通讯桥能回话了（预发布）。** 完整说明见 [RELEASE-NOTES-v0.3.4-beta.1.md](docs/RELEASE-NOTES-v0.3.4-beta.1.md)（英文）。

伴侣现在会把回复自动发回你发消息的 Telegram/Discord 对话（只回主人本人，可选语音条，三档模式：关/总是/对方发语音才回语音）；Telegram 语音消息会用你配置的云语音识别转写后进入对话。新增完全离线的免费本地 TTS（MeloTTS 中英，约 165MB 模型应用内下载）。**注意：allowlist 改为默认拒绝——留空不再接收任何消息，需在设置里填入自己的 Chat/频道 ID。** 另含首跑模型选择的国内/海外/本地分区、更友好的聊天报错文案、webhook 安全加固。预发布版仅供手动验证，稳定版用户不会被自动升级。
