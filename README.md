...
> **主题：记忆不再是黑箱。** 中文说明见 [RELEASE-NOTES-v0.3.5.zh-CN.md](docs/RELEASE-NOTES-v0.3.5.zh-CN.md)，英文完整说明见 [RELEASE-NOTES-v0.3.5.md](docs/RELEASE-NOTES-v0.3.5.md)。

以前 Nexus 可以“记住”，但用户不容易知道它到底想起了什么。0.3.5 开始把记忆摆到台面上：回复会提示是否用了记忆，可以展开看来源，点进 Memory 页还能高亮对应条目；用户也可以暂停全部记忆或单条记忆。这版不把 Nexus 做成替你干活的 Codex 式智能体，而是让陪伴记忆变得本地、可看、可停、可回滚。
