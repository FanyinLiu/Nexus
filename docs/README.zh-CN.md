...
> **主题：记忆看得见，伙伴也动起来。** 这版不把 Nexus 做成替你干活的 Codex 式智能体，而是让陪伴记忆变得本地、可看、可停、可回滚，同时让桌面伙伴的待机、思考、聆听、说话、等待、错误和离线状态可以直接预览。中文说明见 [RELEASE-NOTES-v0.3.5.zh-CN.md](RELEASE-NOTES-v0.3.5.zh-CN.md)，英文完整说明见 [RELEASE-NOTES-v0.3.5.md](RELEASE-NOTES-v0.3.5.md)。

以前 Nexus 可以“记住”，但用户不容易知道它到底想起了什么；也能常驻桌面，但状态不够容易被看懂。0.3.5 开始把两件事摆到台面上：回复会提示是否用了记忆，可以展开看来源，点进 Memory 页还能高亮对应条目；Companion Profile 也能预览桌面伙伴的关键状态。

一句话记住 0.3.5：

- **Nexus 现在能说明这次回复用了哪些记忆。**
- **你可以回到 Memory 页看到、暂停、编辑或删除这些记忆。**
- **桌面伙伴有待机、思考、聆听、说话、等待、错误和离线状态预览。**
