...
## 本次更新 — v0.3.1-beta.3（预发布）

> **bug 修复 patch（无新功能）。** 主要修：dmg/exe 用户的 Live2D 桌宠（CSP 缺 `unsafe-eval` 导致 pixi.js 渲染失败 — P0）；thinking-mode 模型多轮对话（DeepSeek-R1 / QwQ / Hunyuan-thinking 的 reasoning_content 全链路保留 + 旧历史兜底）；TTS 读完后 voiceState 卡 speaking 三层防护；多模态发图后续轮丢失；in-app 诊断面板自动捕获 console + dev 模式 tail 到 `.dev/runtime.log`。完整说明见 [RELEASE-NOTES-v0.3.1-beta.3.md](RELEASE-NOTES-v0.3.1-beta.3.md)（英文）。
