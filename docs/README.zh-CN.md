...
## 本次更新 — v0.3.1-beta.4（预发布）

> **审计 + 打磨 patch。** 修复记忆存储 compaction race（数据丢失风险）；MCP per-tool 批准（M2）+ workspace:set-root 批准（M3）；新增 Settings 「关于 + 帮助」面板和「每周回顾」面板；约 660 条日韩 UI 翻译补齐；唤醒词重试日志去重实际生效；运行时错误转人话措辞。完整说明见 [RELEASE-NOTES-v0.3.1-beta.4.md](RELEASE-NOTES-v0.3.1-beta.4.md)（英文）。
