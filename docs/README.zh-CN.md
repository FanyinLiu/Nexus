...
## 本次更新 — v0.3.4-beta.1（预发布）

> **通讯桥能回话了。** 伴侣自动回信到来源 Telegram/Discord 对话（只回主人，可选语音条），Telegram 语音消息可转写进对话，新增免费离线本地 TTS。**allowlist 改为默认拒绝，留空不再收消息。** 完整说明见 [RELEASE-NOTES-v0.3.4-beta.1.md](RELEASE-NOTES-v0.3.4-beta.1.md)（英文）。预发布版仅供手动验证。
