...
## 今回のアップデート — v0.3.3-beta.1（プレリリース）

> **メンテナンス + 仕上げ + スリム化。** インポートしたキャラクターカードがチャットを駆動、音声認識のクラウド→ローカル フォールバック、各種バグ修正、デフォルトペットを Pip に改名し既定で背景表示、分身機能を削除、約 2,500 行のデッドコードを削減。詳細は [RELEASE-NOTES-v0.3.3-beta.1.md](RELEASE-NOTES-v0.3.3-beta.1.md)（英語）。プレリリースは手動検証用です。
