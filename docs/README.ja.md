...
> **デスクトップ・コンパニオン認識の土台。** この安定版では、Nexus を開いたあとユーザーが別の場所で作業していても、時間の経過を静かに理解するループを始めます。詳細は [RELEASE-NOTES-v0.4.0.md](RELEASE-NOTES-v0.4.0.md)（英語）。

Nexus は静かで保守的、一時停止可能なまま、短期で粗いコンパニオン要約だけを作ります。時間表現は「しばらく」「30分くらい」「1時間くらい」のように粗く、生のスクリーンショット、完全なクリップボード、私的なメッセージ、正確なタイマーはモデルへ送りません。プロアクティブな check-in 拡張は後続版へ、マウス追従、タイピング反応、ウィンドウ操作は v0.5 の範囲です。
...
> **彼女の小さな表情が、チャット文ではなく顔に届くように。** 彼女は返信に括弧で小さな仕草（「（目が輝いた）」「（首をかしげる）」）をよく添えます。これらはアバター用のステージ指示で、文字として読み上げたり表示したりするものではありませんが、時々それが返信に漏れて不具合のように見えていました。今回は**形（シェイプ）**で判断するようにしました（手動で維持する単語リストに頼らず）：認識できる表情は彼女の顔を動かし、表示も読み上げもしません（従来どおり）。未収録の即興のひと言は残しつつ、淡いイタリックの**ト書き**として描画され、漏れた文字ではなく意図した傍白として読めます。どちらも読み上げません（音声ストリーミングの途中でも）。コロン付きの「（注：月曜は通常どおり）」は本文として従来どおり表示します。**形**で判断するため、他の人の画面に出る新しい表情も同じように扱われ、リストに追加する必要はありません。詳細は [RELEASE-NOTES-v0.3.4-beta.4.md](RELEASE-NOTES-v0.3.4-beta.4.md)（英語）。プレリリースは手動検証用です。

## 今回のアップデート — v0.3.4-beta.3（プレリリース）

> **彼女の感情が、説明ではなく行動を動かすように。** 感情エンジンをエンドツーエンドで接続：会話全体からあなたの気持ちを読み取り（「今日リストラされた」にもキーワードなしで気づく）、彼女自身の感情は実際の出来事（返信失敗・長い不在・あなたの落ち込んだ日々）で形作られ、それが「どう・声をかけるかどうか」を実際に変えます（心配なら そっと様子を、疲れていれば静かに、ただしサイレント時間は越えず、機嫌が良くても饒舌にはならない、近いほど素直に）。さらに Live2D 星絵が既定アバターに、メッセージ感知がワントグルに、見逃したメッセージは次の会話でそっと。詳細は [RELEASE-NOTES-v0.3.4-beta.3.md](RELEASE-NOTES-v0.3.4-beta.3.md)（英語）。プレリリースは手動検証用です。

## 今回のアップデート — v0.3.4-beta.2（プレリリース）

> **コンパニオンがすべてのメッセージを感知。** デスクトップメッセージ感知（macOS）を追加：システムの通知センターを読み取り、WeChat/QQ/メールなどあらゆるアプリの新着をコンパニオンが把握して会話に反映します（既定では送信元+差出人のみ。「フルディスクアクセス」権限が必要）。ブリッジにペアリングコード認証を追加（Chat ID の手入力が不要に）。MiniMax ウェブ検索、設定のモデル地域タブ、約 2,100 行のデッドコード削除も含みます。beta.1 の全内容を含みます。詳細は [RELEASE-NOTES-v0.3.4-beta.2.md](RELEASE-NOTES-v0.3.4-beta.2.md)（英語）。プレリリースは手動検証用です。

## 今回のアップデート — v0.3.3

> **メンテナンス + 仕上げ + スリム化。** インポートしたキャラクターカードがチャットを駆動、音声認識のクラウド→ローカル フォールバック、各種バグ修正、デフォルトペットを Pip に改名し既定で背景表示、分身機能を削除、約 2,500 行のデッドコードを削減。詳細は [RELEASE-NOTES-v0.3.3.md](RELEASE-NOTES-v0.3.3.md)（英語）。v0.3.3-beta.1 の検証期間を経て安定版に昇格、内容は同一です。
...
| 0.4 安定版を見る | [v0.4.0 Release Notes](RELEASE-NOTES-v0.4.0.md) · [v0.4 Release Hardening](RELEASE-CANDIDATE-v0.4-HARDENING.md) |
...
コミュニティ文書は特定の 1 リリースだけのものではなく、全バージョンをまたぐ入口です。0.3 は安全性、メモリ、設定の土台を締め、0.4 はデスクトップ・コンパニオン認識へ進み、0.5 はデスクトップペットの振る舞いへ進みます。長期入口は [Community Guide](COMMUNITY.md)、0.4 の方向は [v0.4 Desktop Companion Awareness](V0.4_DESKTOP_COMPANION_AWARENESS.md)、0.4 安定版の説明は [v0.4.0 Release Notes](RELEASE-NOTES-v0.4.0.md)、リリース前の hardening は [v0.4 Release Hardening](RELEASE-CANDIDATE-v0.4-HARDENING.md) です。
