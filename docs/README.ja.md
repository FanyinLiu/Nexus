...
> **寄り添い、覚えていく Nexus の土台を固める版。** このリリースは Nexus を Codex のような作業エージェントにするものではありません。コンパニオン体験に必要な信頼の土台を整えます：初回起動とモデル接続の修復が分かりやすくなり、リリース/更新の状態が明確になり、IPC・権限・鍵・監査の境界が強化されました。チャット記憶はメインプロセス SQLite への隠し移行リハーサルに入りました。既存チャットは引き続き localStorage が権威ソースで、削除されません。隠しリハーサルでは dry-run、バックアップ、適用、ロールバック、実行時ミラー、メタデータ照合ができます。詳細は [RELEASE-NOTES-v0.3.5.md](RELEASE-NOTES-v0.3.5.md)（英語）。
