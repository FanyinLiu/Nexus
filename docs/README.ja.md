...
## 今回のアップデート — v0.3.4-beta.1（プレリリース）

> **メッセージブリッジが返事をするように。** コンパニオンの返信が元の Telegram/Discord チャットに自動で届きます（オーナーのみ、ボイスメッセージも選択可）。Telegram のボイスメッセージは文字起こしされて会話に入ります。無料のオフライン ローカル TTS を追加。**allowlist はデフォルト拒否に変更——空欄では何も受信しません。** 詳細は [RELEASE-NOTES-v0.3.4-beta.1.md](RELEASE-NOTES-v0.3.4-beta.1.md)（英語）。プレリリースは手動検証用です。
