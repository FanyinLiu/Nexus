...
| Chatbox | Desktop composer ergonomics: input is obvious, aligned, and central to the task. | Floating action clusters, generic desktop-client chrome, or oversized shortcut bars. | Image4 composer, chat composer, and attachment/send alignment; voice controls remain in Settings. |
...
| Composer | Chatbox desktop composer ergonomics, Jan desktop locality, assistant-ui composer state, LibreChat capability-entry boundaries, plus Vercel streaming priority. | Textarea, send, and attachment controls share one center line; voice controls stay in Settings. Embedded tools default to icon-only inside the input and reveal button feedback on hover, focus, or enabled-send states. Advanced inputs stay discoverable without making the composer a platform toolbar. | Floating action clusters, resident voice buttons, persistent tool backplates inside the input, browser-page shell assumptions, ChatGPT-like input skin, library demo skin, and changing control sizes to fix alignment. |
...
| Input/composer | Chatbox, Vercel AI Chatbot, assistant-ui, Jan, LibreChat | Composer remains the primary action anchor while state, attachments, runtime metadata, and advanced capability entry stay aligned and quiet. Voice runtime controls stay in Settings. Embedded tools can be visible as icons without default tile backplates. | No resident voice button, floating action clusters, persistent mini-button rows inside the input, browser shell spacing, ChatGPT-like input skin, or control resizing as an alignment fix. |
...
| --- | --- | --- |
| Image4 panel | The header / Live2D stage / recap / composer structure remains intact, the composer stays reachable, and retired dial/greeting/action chrome does not return. | `image4:contract:check`, `image4:contract:report`, the live `PanelView.tsx` route, and current-build screenshots. |
...
| Live2D stage | The selected companion remains the visual anchor and state changes do not displace the composer or recap. | Current panel screenshots, geometry checks, and Live2D runtime evidence. |
...
| Conversation recap | The latest user turn and matching assistant reply remain compact, readable, and correctly paired. | Panel contract tests plus current browser review. |
| Composer | Textarea, send, and attachment controls share a center line; voice controls stay in Settings; embedded tools feel inside the input by default; focus is visible and does not resize the row or elevate the row wrapper. | `composer:surface:audit`, `image4:visual-contract:audit`, human review, optional box alignment measurement. |
...
| --- | --- | --- |
| Image4 | Voice-first four-part rhythm: header, Live2D stage, conversation recap, composer. | Generic dashboard grids, retired dial/greeting/action chrome, or card stacks. |
...
- Evidence to check: `composer:surface:audit`, `docs/COMPOSER_SURFACE_REFERENCE_REVIEW.md`, send/attachment centers align with the textarea, no resident voice control returns, default tool state blends into the input, and focus does not shift layout.
...
| `composer` | 2 | Chatbox, Jan, assistant-ui, LibreChat, Vercel AI Chatbot | Input primacy, state separation, runtime metadata restraint, send/attachment alignment, Settings-owned voice controls, advanced capability entry, progressive replies. |
