...
> **桌面陪伴感知地基。** 這個穩定版開始建立「打開 Nexus 後，它能安靜理解時間流逝」的循環。完整說明見 [RELEASE-NOTES-v0.4.0.md](RELEASE-NOTES-v0.4.0.md)（英文）。

Nexus 會保持安靜、保守、可暫停，只形成短期粗粒度的陪伴摘要。它使用「一會兒」「半小時左右」「一小時左右」這類粗略時間，不把原始截圖、完整剪貼簿、私人訊息或精確計時送進模型。主動 check-in 擴展留到後續版本，0.5 才會做桌寵跟隨滑鼠、打字反應和視窗互動。
...
> **她的情緒現在真的驅動她了，不只是描述她。** 情緒引擎端到端打通：她從整段對話讀出你的心情（「我今天被裁了」也能察覺），自己的情緒由真實事件塑造（回覆失敗/你長時間離開/你最近不開心），並真的改變她怎麼、要不要開口（擔心→輕輕確認，累→安靜，絕不越過免打擾，心情好≠話癆，關係越近越開放）。另含 Live2D 星繪成預設形象、訊息感知合併成一個開關、錯過的訊息下次對話溫柔帶一句。完整說明見 [RELEASE-NOTES-v0.3.4-beta.3.md](RELEASE-NOTES-v0.3.4-beta.3.md)（英文）。預發布版僅供手動驗證。

## 本次更新 — v0.3.4-beta.2（預發布）

> **伴侶能感知你全部的訊息了。** 新增桌面訊息感知（macOS）：讀系統通知中心，微信/QQ/郵件等任何應用來信伴侶都知道、會進對話（預設只報來源+發件人，需「完整磁碟取用」權限）。通訊橋新增配對碼授權（不用手挖 Chat ID），另含 MiniMax 聯網搜尋、設定頁模型分區、刪 2100 行死碼。含 beta.1 全部內容。完整說明見 [RELEASE-NOTES-v0.3.4-beta.2.md](RELEASE-NOTES-v0.3.4-beta.2.md)（英文）。預發布版僅供手動驗證。

## 本次更新 — v0.3.3

> **維護 + 打磨 + 瘦身。** 匯入角色卡可驅動聊天、語音識別雲→本地備援、一批 bug 修復、桌寵預設改名 Pip 並預設顯示背景、移除分身、刪約 2500 行死碼。完整說明見 [RELEASE-NOTES-v0.3.3.md](RELEASE-NOTES-v0.3.3.md)（英文）。v0.3.3-beta.1 驗證期結束後轉正，內容一致。
...
| **🐛 30+ 靜態 bug 修復** | 四輪 audit + 並行靜態掃描；template-replace `$&` 漏洞、並發競態、StrictMode 純淨化、NaN 守衛、async leak、storage 驗證。 |
...
- **下載來源**：只從 [GitHub Releases](https://github.com/FanyinLiu/Nexus/releases/latest) 下載。不要從鏡像或轉載壓縮包安裝；如果你不確定檔案來源，請刪除後重新從 GitHub Releases 下載。
...
社群文件會貫穿所有版本，不只屬於某一次發布。0.3 收安全、記憶和設定地基；0.4 進入桌面陪伴感知；0.5 進入桌寵桌面行為。長期入口見 [Community Guide](COMMUNITY.md)，0.4 方向見 [v0.4 Desktop Companion Awareness](V0.4_DESKTOP_COMPANION_AWARENESS.md)，0.4 正式版說明見 [v0.4.0 Release Notes](RELEASE-NOTES-v0.4.0.md)，發布前加固清單見 [v0.4 Release Hardening](RELEASE-CANDIDATE-v0.4-HARDENING.md)。
