...
## 本次更新 — v0.3.1-beta.4（預發布）

> **審計 + 打磨 patch。** 修復記憶存儲 compaction race（資料丟失風險）；MCP per-tool 批准（M2）+ workspace:set-root 批准（M3）；新增 Settings 「關於 + 幫助」面板和「每週回顧」面板；約 660 條日韓 UI 翻譯補齊；喚醒詞重試日誌去重實際生效；執行時錯誤轉人話措辭。完整說明見 [RELEASE-NOTES-v0.3.1-beta.4.md](RELEASE-NOTES-v0.3.1-beta.4.md)（英文）。
