...
> **預發布版本。** 不會自動推送給 v0.2.9 及以下穩定版使用者 —— 需要從
> GitHub Releases 手動下載。下個穩定版 v0.3.0 發布時，beta 使用者會
> 自動升級。完整開發者視角說明見
> [RELEASE-NOTES-v0.3.0-beta.1.md](RELEASE-NOTES-v0.3.0-beta.1.md)（英文）。

關係演化 + 情感記憶沿三條獨立軸深化：

- **💝 情緒共鳴召回** —— 當前心情影響召回哪些記憶。VAD（valence /
  arousal）投影 + 三種調節模式：匹配（預設）/ 共情（"陪陪我" 觸發）/
  修復（"算了 換個話題"、持續嚴重低落觸發）。附帶 priming 環形緩衝
  防止情感跳躍。
- **🎯 關係升級儀式 + 重逢增強** —— 第一次跨越 10/30/55/80 分數閾值
  時，觸發**一次性**指令讓模型"表現出"變化而非"宣告"變化。久別
  重逢時自動引用上次話題 + 情緒，在 close_friend 以上關係且上次
  concern 高時會溫柔詢問"事情好轉了嗎"。
- **🌳 關係子維度** —— 單一 score 拆為四個命名維度：trust（信任）/
  vulnerability（脆弱開放）/ playfulness（玩鬧默契）/ intellectual
  （思維深度）。每維度從不同類型的對話中生長，有遞減增益，長期
  無互動時緩慢衰減。明顯偏高 / 偏低時各自注入**具體**提示指令。

**底層改動：** Electron 36 → 41（Node 24 ABI）；93 個新單元測試把
總覆蓋從 486 提升到 665；主程序安全加固（CSP 收緊、vault 檔案
0o600 權限、子程序 PID 守衛、多處 timer leak 修復）；共享
`driftToward()` / `classifyByPatterns()` 工具去重。
