...
## 本次更新 — v0.3.4-beta.1（預發布）

> **通訊橋能回話了。** 伴侶自動回信到來源 Telegram/Discord 對話（只回主人，可選語音訊息），Telegram 語音訊息可轉寫進對話，新增免費離線本地 TTS。**allowlist 改為預設拒絕，留空不再收訊息。** 完整說明見 [RELEASE-NOTES-v0.3.4-beta.1.md](RELEASE-NOTES-v0.3.4-beta.1.md)（英文）。預發布版僅供手動驗證。
