...
5. i18n locale 已按域拆目录（P2），新增文案键要进对应域文件，不要新建单文件 locale。zh-CN 是 key 集合的唯一真源：`TranslationKey`（`src/types/i18n.ts`）由 `keyof typeof zhCNMessages` 派生，无手写 union；其余 4 个 locale 域文件 `as const satisfies Partial<TranslationDictionary>`、聚合字典 `satisfies TranslationDictionary`，tsc 编译期把它们钉死在 zh-CN key 集合上；zh-CN 域文件只 `satisfies Record<string, string>`（从自身字典反引派生 key 集会构成类型循环）——加/删 key 只需改 zh-CN 对应域文件 + 其余 4 个 locale 的对应域文件，无生成步骤；`verify:pr` 里的 `i18n:audit` 负责其他 locale 对 zh-CN 的 parity。
