...
> **곁에 있고 기억하는 Nexus의 기반을 다지는 버전.** 이번 릴리스는 Nexus를 Codex 같은 작업 에이전트로 만드는 것이 아닙니다. 동반자 경험에 필요한 신뢰 기반을 보강합니다: 첫 실행과 모델 연결 문제를 더 쉽게 고치고, 릴리스/업데이트 상태를 더 명확히 하며, IPC·권한·키·감사 경계를 강화했습니다. 채팅 기억은 메인 프로세스 SQLite로 옮기기 위한 숨겨진 마이그레이션 리허설에 들어갔습니다. 기존 채팅은 계속 localStorage가 기준 저장소이며 삭제되지 않습니다. 숨겨진 리허설은 dry-run, 백업, 적용, 롤백, 런타임 미러, 메타데이터 비교를 지원합니다. 자세한 내용은 [RELEASE-NOTES-v0.3.5.md](RELEASE-NOTES-v0.3.5.md)(영어).
