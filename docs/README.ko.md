...
## 이번 업데이트 — v0.3.4-beta.1(프리릴리스)

> **메시징 브리지가 답장을 합니다.** 컴패니언의 답장이 원본 Telegram/Discord 대화로 자동 전송됩니다(주인에게만, 음성 메시지 선택 가능). Telegram 음성 메시지는 전사되어 대화에 들어갑니다. 무료 오프라인 로컬 TTS 추가. **allowlist가 기본 거부로 변경 — 비워두면 아무것도 받지 않습니다.** 자세한 내용은 [RELEASE-NOTES-v0.3.4-beta.1.md](RELEASE-NOTES-v0.3.4-beta.1.md)(영어). 프리릴리스는 수동 검증용입니다.
...
- [ ] **Decision / Roleplay / 백그라운드 작업 분리** — 의도 분류(빠름), 롤플레이(페르소나 순수), 백그라운드 작업 실행을 분리. 롤플레이 계층은 도구 메타데이터를 전혀 보지 않으며, 작업 결과는 캐릭터가 자기 목소리로 "전달"합니다.
