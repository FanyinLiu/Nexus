...
## 이번 업데이트 — v0.3.3-beta.1(프리릴리스)

> **유지보수 + 다듬기 + 슬리밍.** 가져온 캐릭터 카드가 채팅을 구동, 음성 인식 클라우드→로컬 폴백, 다수 버그 수정, 기본 펫 이름을 Pip으로 변경하고 기본적으로 배경 표시, 분신 기능 제거, 약 2,500줄의 데드 코드 제거. 자세한 내용은 [RELEASE-NOTES-v0.3.3-beta.1.md](RELEASE-NOTES-v0.3.3-beta.1.md)(영어). 프리릴리스는 수동 검증용입니다.
