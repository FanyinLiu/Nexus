...
## 이번 업데이트 — v0.3.0-beta.1

> **프리릴리즈 버전.** v0.2.9 이하 안정 버전 사용자에게는 자동 배포되지
> 않습니다 —— GitHub Releases에서 수동으로 다운로드해야 합니다. 다음
> 안정 버전 v0.3.0이 출시되면 베타 사용자는 자동으로 업데이트됩니다.
> 개발자 관점의 전체 설명은
> [RELEASE-NOTES-v0.3.0-beta.1.md](RELEASE-NOTES-v0.3.0-beta.1.md)(영어)를
> 참조하세요.

관계 진화 + 감정 메모리가 세 개의 독립적 축을 따라 심화:

- **💝 감정 공명 리콜** —— 현재 기분이 어떤 메모리가 떠오를지에 영향.
  VAD(valence / arousal) 투영 + 세 가지 조절 모드: 강화(기본) /
  공감("listen to me" / "陪陪我" 트리거) / 복구("move on" / 지속적
  심한 고통 트리거). priming 링 버퍼로 감정 급변 방지.
- **🎯 관계 마일스톤 + 재회 강화** —— 점수 임계값 10/30/55/80을 처음
  넘을 때 **일회성** 지시를 발화시켜 변화를 "연기"하도록(선언하지
  않고) 유도. 오랜만의 재회 시 지난 화제 + 감정을 자연스럽게 짜
  넣고, close_friend 이상에서 지난 concern이 높았다면 "그 일은
  좀 나아졌어?"라고 부드럽게 물어봄.
- **🌳 관계 서브 차원** —— 평탄한 점수가 네 개의 명명된 차원으로
  분해: trust(신뢰) / vulnerability(개방성) / playfulness(장난스러움)
  / intellectual(지적 깊이). 각 차원은 서로 다른 종류의 대화에서
  성장하며, 체감성 효과가 있고, 장기 미상호작용 시 서서히 쇠퇴.
  두드러지게 높거나 낮을 때 각각 **구체적인** 프롬프트 지시를 주입.

**내부 변경:** Electron 36 → 41(Node 24 ABI); 93개의 새 유닛 테스트가
총 커버리지를 486에서 665로 확대; 메인 프로세스 보안 강화(CSP
축소, vault 파일 0o600 권한, 자식 프로세스 PID 가드, 여러 timer
leak 수정); 공유 `driftToward()` / `classifyByPatterns()` 유틸로
중복 제거.
