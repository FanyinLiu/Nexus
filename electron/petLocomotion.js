...
// A small main-process state machine walks the pet window along its current
...
//
// Decoupled from windowManager via injected accessors (no circular import):
//   getWin()      -> the pet BrowserWindow (or null)
//   getState()    -> the current petWindowState object
//   patchState(p) -> merge p into petWindowState and broadcast it
...
export function configurePetLocomotion({ getWin: win, getState: state, patchState: patch }) {
  getWin = win
  getState = state
  patchState = patch
}
...
export function setPetFreeMode(on) {
...
export function notePetDrag(delta) {
