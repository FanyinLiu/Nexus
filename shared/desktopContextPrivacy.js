/**
 * Desktop context privacy helpers — single source of truth shared by the
 * Electron main process (desktopContextPrivacy.js) and the Vite renderer
 * (src/lib/privacy/desktopContextPrivacy.ts). Captured window titles,
 * clipboard text, and OCR/VLM output must pass through these checks before
 * leaving the main process or reaching a prompt.
 */
...
function containsSensitiveDesktopContext(value) {
...
function redactSensitiveDesktopContextText(value) {
  const text = String(value ?? '').trim()
  if (!text) return ''
